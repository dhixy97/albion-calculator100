...
export default function PriceMaterials({localReturnRate}) {
  ...
  const {selectedItem, addRecipe, cityBuyMats, setCityBuyMats, setSelectedItem, setRecipes, sellCity, setSellCity, clearAll} = useRecipes()
    ...
    <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
      ...
      <div className="grid grid-cols-4 gap-3 mt-2">
        ...
        <div className="flex flex-col">
          ...
          <select className="bg-slate-700 text-gray-200 px-3 py-2 rounded-md border border-slate-600 outline-none">
            {cities.map((c) => (
                <option key={c} value={c}>
                    {c}
                </option>
      ...
      <div className="mt-2 border-t">
        <div className="mt-2 flex gap-5">
            <button className="bg-blue-600 px-4 py-2 rounded-md hover:bg-blue-700 transition cursor-pointer" onClick={handleAddRecipe}>Add Recipes</button>
            <button className="bg-red-600 px-4 py-2 rounded-md hover:bg-red-900 transition cursor-pointer" onClick={() => clearAll()}>Clear All</button>
        </div>
      </div>
    </div>
  );
